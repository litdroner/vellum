...
| 2026-09-24 | Vellum Flow V1 (owner request, 1.0 fast track): a workflow is a saved, named list of steps, each an operation id and its settings (never a tool or command id), kept on this PC in workflows.json with no paths or document content. It runs on the files the person picks as a batch: steps in order per file, one file at a time, in memory in between, only the last step writing a new file; a failed step writes nothing for that file. V1's steps are the operations that exist: Office → PDF (first only), page numbers, text watermark, compress. No branching, loops, scripts, schedules or network steps; task history stays planned (§3.10) |
