...
- **Current phase: 0.5.0 object editing, not complete.** Plan and phase records:
  ...
  below). Of the should-haves, alignment, distribution, snapping, image replacement and image insertion are done on `main`;
...
|---|---|---|
| 0.5.0 should-haves left, each only if its strict tests pass (otherwise they move to a later release): paragraph grouping, overlap warnings, single-style paragraph reflow (last, gated) | next | §3.2 |
