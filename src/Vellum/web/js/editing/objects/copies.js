...
//
//   { id, kind: 'text-copy', entry, target: { key, text, glyphs }, text, encoding, transform }
//   { id, kind: 'image-copy', entry, target: { key, stream, opIndex, name, inline, ctm, width, height },
...
//
// The page a copy is on must draw the same original content as the page it was copied from — that
// page, or a duplicate of it — because that content is what the copy draws from. A picture put on a
// page from a file needs none of this: its record holds everything, so a copy of it is simply another
// inserted-image record (inserted-image.js), which can go on any page.
//
...
import { num, operand, pdfName } from '../content/writer.js';
import { IDENTITY, multiply } from '../matrix.js';
import { determinant, isValid, quantize } from './transform.js';
import { addStandardFont, drawText, encodeStandard, originalItems, sameGlyphs } from './text-run.js';
 ...
 */
export function planCopy({ kind, target, text, encoding, replacement = null, transform, entry, id = newId() }) {
  ...
  if (kind === TEXT) {
    ...
    return structuredClone({ id, kind, entry, target, text, encoding, transform: kept });
  }
  if (kind === IMAGE) {
    ...
    return structuredClone({ id, kind, entry, target, transform: kept, ...(replacement ? { replacement: replacementOf(replacement) } : {}) });
  }
...
export const textCopy = Object.freeze({
  ...
  write({ lib, doc, page, index, analysis, records }) {
    ...
    const append = records.map((record) => {
      ...
      return drawText(analysis, run, fontName, items, record.transform);
    });
...
export const imageCopy = Object.freeze({
  ...
  write({ lib, doc, page, index, analysis, records, prepared }) {
    const draws = records.map((record) => {
      ...
      return { image, placed, ref };
    });
    const names = updateResources(lib, doc, page, analysis, [], draws.filter((d) => d.ref).map((d) => d.ref));
    let next = 0;
    const append = draws.map(({ image, placed, ref }) => {
      const replay = [image.fill?.space, image.fill?.color].filter(Boolean).map(({ op, args }) => `${args.map(operand).join(' ')} ${op}`.trim());
      const states = (image.gsNames ?? []).map((name) => `${pdfName(name)} gs`);
      const name = ref ? names[next++] : image.name;
